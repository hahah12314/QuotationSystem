<template>
  ...
  <el-card class="box-card">
    ...
    <el-dialog :title="this.dataDialogForm.roleId===0 ? '新增角色' : '更新角色' " :visible.sync="dialogFormVisible" width="35%"
      @close="closeDialog">
      <el-form :model="dataDialogForm" :rules="rules" ref="ruleForm" size="mini">
        ...
        <el-form-item label="描述信息" :label-width="formLabelWidth" prop="remark">
          <el-input v-model="dataDialogForm.remark" autocomplete="off" type="textarea" style="width: 240px;"></el-input>
        </el-form-item>
        <el-form-item label="菜单权限" :label-width="formLabelWidth" prop="menuList">
          <el-tree :data="menuAll" show-checkbox node-key="menuId" :props="defaultProps" ref="tree" check-strictly>
          </el-tree>
        </el-form-item>
      </el-form>
    ...
    methods: {
      ...
      async handleEdit(index, item) {
        ...
        try {
          ...
          this.checks = res.data.data; // 这里假设返回的是一个数组，包含实际选中的菜单ID
      ...
      async updateRole(formName) {
        ...
        try {
          ...
          let halfCheckedKeys = this.$refs.tree.getHalfCheckedKeys(); // 半选状态的节点，即父节点
          let allSelectedKeys = [...checkedKeys, ...halfCheckedKeys];

          // 去重
          this.dataDialogForm.menuList = Array.from(new Set(allSelectedKeys));
