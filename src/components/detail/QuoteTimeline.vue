<template>
    <div class="timeline-container">
      <el-timeline class="horizontal-timeline">
        <el-timeline-item
          ...
          :key="index"
          :icon="getIcon(activity, index)"
          :type="getType(activity, index)"
          :color="getColor(activity, index)"
          :size="activity.size"
  ...
  .horizontal-timeline .el-timeline-item {
    flex: 1;
    margin-right: 20px; /* 调整每个时间线项之间的间距 */
    text-align: center;
  }
  
  .horizontal-timeline .el-timeline-item:last-child {
    margin-right: 0; /* 最后一个时间线项不需要右边距 */
  }
  ...
  .horizontal-timeline .el-timeline-item__node {
    ...
    top: 0;
    left: 0;
    margin-right: 10px; /* 调整节点与内容之间的间距 */
    width: 24px;
    height: 24px;
    border: none;
    box-shadow: 0 2px 12px 0 rgba(0, 0, 0, 0.1);
