      ...
      <view class="card-container">
        <view class="material-card" v-for="(material, index) in nowDataList" :key="material.id">
          ...
          <view class="card-body">
            <text>原材料长度: {{ material.length }}</text>
            <text>宽度: {{ material.width }}</text>
            <text>高度: {{ material.height }}</text>
            <text>毛坯重量: {{ material.blankWeight }}</text>
            <text>材料单价: {{ material.materialUnitPrice }}</text>
            <text>毛坯费用: {{ material.blankFee }}</text>
            <text>废料重量: {{ material.scrapWeight }}</text>
            <text>废料单价: {{ material.scrapUnitPrice }}</text>
            <text>废料费用: {{ material.scrapFee }}</text>
            <text>总材料成本: {{ material.totalMaterialCost }}</text>
          </view>
...
export default {
  data() {
    return {
      ...
      sortOptions: ['默认', '总材料成本升序', '总材料成本降序'],
    };
  },
  methods: {
    async fetchAllData() {
      ...
      try {
        while (currentPage <= totalPages) {
          const res = await uni.$http.get('/material-cost/list', { pageIndex: currentPage, pageSize: 1000 });
    ...
    applyFilters() {
      ...
      if (this.filters.sortBy > 0) {
        const sortFunctions = {
          1: (a, b) => a.totalMaterialCost - b.totalMaterialCost,
          2: (a, b) => b.totalMaterialCost - a.totalMaterialCost,
        };
