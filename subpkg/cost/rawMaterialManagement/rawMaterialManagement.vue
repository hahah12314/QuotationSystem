  ...
  <view class="container">
    <view class="uni-container">
      ...
      <view class="card-container">
        <view class="material-card" v-for="(rawMaterial, index) in nowDataList" :key="rawMaterial.materialId">
          <view class="card-header">
            <text class="material-name">{{ rawMaterial.specification }}</text>
            <text class="material-index">#{{ (pageIndex - 1) * pageSize + index + 1 }}</text>
          </view>
          <view class="card-body">
            <text>材料规格: {{ rawMaterial.specification }}</text>
            <text>单位重量 (公斤): {{ rawMaterial.unitWeight }} 公斤</text>
            <text>净重 (公斤): {{ rawMaterial.netWeight }} 公斤</text>
            <text>利润: {{ rawMaterial.profit }} 元</text>
            <text>杂费: {{ rawMaterial.miscellaneousFees }} 元</text>
            <text>税前总价: {{ rawMaterial.preTax }} 元</text>
            <text>产品总价: {{ rawMaterial.pricePerkg }} 元</text>
            <text>创建时间: {{ rawMaterial.createTime }}</text>
            <text>更新时间: {{ rawMaterial.updateTime }}</text>
          </view>
          <!-- <view class="card-footer">
            <button class="custom-button edit" @click="handleEdit(rawMaterial)">编辑</button>
            <button class="custom-button delete" @click="handleDelete(rawMaterial)">删除</button>
          </view> -->
        </view>
      </view>
  ...
  methods: {
    async fetchAllData() {
      ...
      try {
        while (currentPage <= totalPages) {
          const res = await uni.$http.get('/raw-materials/list', {
    ...
    applyFilters() {
      ...
      if (this.filters.sortBy > 0) {
        const sortFunctions = {
          1: (a, b) => a.pricePerkg - b.pricePerkg,
          2: (a, b) => b.pricePerkg - a.pricePerkg,
          3: (a, b) => a.preTax - b.preTax,
          4: (a, b) => b.preTax - a.preTax,
          5: (a, b) => a.profit - b.profit,
          6: (a, b) => b.profit - a.profit,
          7: (a, b) => a.createTime.localeCompare(b.createTime),
          8: (a, b) => b.createTime.localeCompare(a.createTime),
          9: (a, b) => a.updateTime.localeCompare(b.updateTime),
          10: (a, b) => b.updateTime.localeCompare(a.updateTime)
        };
    ...
    handleEdit(rawMaterial) {
      // 编辑逻辑
      console.log('编辑:', rawMaterial);
    },
    handleDelete(rawMaterial) {
      // 删除逻辑
      console.log('删除:', rawMaterial);
    }
  },
  onLoad() {
    this.fetchAllData();
